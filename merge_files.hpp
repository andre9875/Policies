...
#ifndef MERGE_FILES_HPP_
#define MERGE_FILES_HPP_
...
#include "FilePolicyPtr.h"
#include "ProcessFilePtr.h"
#include "DirPolicy.h"
...
typedef nxm::ProcessFilePtr<nxm::File,nxm::SplitPolicy> processFile;
...
void merge_files(const vecStr &mergeFiles, const std::string &fileout,const std::string &dir){
	...
	std::fstream ptrWrite;
	processFile FileOut(&ptrWrite); //write result to the final file
	FileOut.OpenWrite(dir + fileout); //create write file

	vecFiles filePointers; //keep list of fstream pointers
	vecProcessFile fileInstances; //keep list of assigned fstream pointers
	...
	for(unsigned int i =0; i < mergeFiles.size(); ++i){
		...
		{
			processFile tmpFile(filePointers.at(i).get()); //create instance of processFile
